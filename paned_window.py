...
class PanedWindow(Tk_PanedWindow):
    ...
    def _on_release(self, event):
        ...
        callback_id1 = self._list_of_panes[handle_index + 1].bind("<Configure>", lambda event,
                                                                  handle_index=handle_index: self._on_configure_pane(
                                                                  handle_index), "+")
        callback_id2 = self._list_of_panes[handle_index].bind("<Configure>", lambda event,
                                                              handle_index=handle_index: self._on_configure_pane(
                                                              handle_index), "+")
    ...
    def add(self, pane, **kwargs):
        ...
        if quantity_of_panes >= 2:
            ...
            callback_id1 = pane.bind("<Configure>",
                                     ...
                                     "+")
            callback_id2 = self._list_of_panes[handle_index].bind("<Configure>", lambda event,
                                                                  handle_index=handle_index: self._on_configure_pane(handle_index), "+")
...
class VerticalPanedWindow(PanedWindow):
    ORIENT = VERTICAL
...
class HorizontalPanedWindow(PanedWindow):
    ORIENT = HORIZONTAL
...
if __name__ == "__main__":
    ...
    # image_vertical = PhotoImage(data="R0lGODlhAwAYAPIAAEBAQGBgYICAgLu7u8zMzAAAAAAAAAAAACH5BAEAAAUALAAAAAADABgAAAMXCCFRI4OUSRVzUNJp24sbt3hZWHQYOCUAOw==")

    image_horizontal = PhotoImage(
        data="R0lGODlhGAADAPIFAEBAQGBgYICAgLu7u8zMzAAAAAAAAAAAACH5BAEAAAUALAAAAAAYAAMAAAMaWBJQym61N2UZJTisb96fpxGD4JBmgZ4lKyQAOw==")

    paned_window = VerticalPanedWindow(root, sashpad=2, image=image_horizontal)
    paned_window.pack(fill="both", expand=True)

    for color in ("red", "blue", "green"):
        frame = Frame(paned_window, width=200, height=200, bg=color)
        paned_window.add(frame, stretch="always")
