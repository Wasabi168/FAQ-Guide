...
def get_device_type(chr_dll, handle: int):
    """
    Get device type.
    :param chr_dll: Handle to chrocodile client lib
    :type chr_dll: CDLL
    ...
    :return: negative error or warning code;
            -1: Unspecified sensor
            1: CHRocodile 1 device
            2: CHRocodile Multi-Channel Device
            3: CHRocodile Compact Device
    :rtype: c_int
    """
    chr_dll.GetDeviceType.restype = c_int
    chr_dll.GetDeviceType.argtypes = [c_int]
    return chr_dll.GetDeviceType(handle)


def get_device_channel_count(chr_dll, handle: int) -> int:
    """
    ...
    :type handle: int
    :return: Negative: error or warning code
            1: CHRocodile 1, CHRocodile C, CHRocodile 2
            n: multi-channel devices, number of measurement channels (MPS) / points (CLS, CLS2.0).
    :rtype: c_int
    """
    chr_dll.GetDeviceChannelCount.restype = c_int
    chr_dll.GetDeviceChannelCount.argtypes = [c_int]
    return chr_dll.GetDeviceChannelCount(handle)


def get_device_connection_info(chr_dll, handle: int, max_buf_sz: int = 1024):
    chr_dll.GetDeviceConnectionInfo.argtypes = [c_int, c_char_p, POINTER(c_int64)]
    chr_dll.GetDeviceConnectionInfo.restype = c_int
    str_buffer = create_string_buffer(max_buf_sz)
    s64 = c_int64(max_buf_sz)
    psz = POINTER(c_int64)(s64)
    err = chr_dll.GetDeviceConnectionInfo(handle, str_buffer, psz)
    resp_str = str_buffer.value.decode('utf-8')
    return resp_str, err


def start_chr_device_auto_search(chr_dll, conn_type: int, blocking_search: int = 0):
    chr_dll.StartCHRDeviceAutoSearch.argtypes = [c_int, c_int]
    chr_dll.StartCHRDeviceAutoSearch.restype = c_int
    err = chr_dll.StartCHRDeviceAutoSearch(conn_type, blocking_search)
    return err


def detected_chr_device_info(chr_dll, max_buf_sz: int = 1024):
    chr_dll.DetectedCHRDeviceInfo.argtypes = [c_char_p, POINTER(c_int64)]
    chr_dll.DetectedCHRDeviceInfo.restype = c_int
    str_buffer = create_string_buffer(max_buf_sz)
    s64 = c_int64(max_buf_sz)
    psz = POINTER(c_int64)(s64)
    err = chr_dll.DetectedCHRDeviceInfo(str_buffer, psz)
    resp_str = str_buffer.value.decode('utf-8')
    return resp_str, err


def is_chr_device_auto_search_finished(chr_dll):
    chr_dll.IsCHRDeviceAutoSearchFinished.restype = c_int
    status = chr_dll.IsCHRDeviceAutoSearchFinished()
    return status


def cancel_chr_device_auto_search(chr_dll) -> None:
    chr_dll.CancelCHRDeviceAutoSearch()
